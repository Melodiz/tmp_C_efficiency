...
import c257_coherent_residual_family_miner
...
import c269_adaptive_length_gate_audit
import c270_adaptive_length_paired_aggregate
...
import c315_failure_gated_512_repetition_penalty
import c316_short_target_anchor_mixed_smoke
import c317_qwen3_30b_a3b_fp8_greedy
...
import c343_compressed_reference_sft_ultramicro_cap_repair
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    if args.id == "C257":
        return c257_coherent_residual_family_miner.run(forwarded)
    if args.id == "C258":
        return c258_c111_family_stratified_validation.run(forwarded)
    if args.id == "C259":
    ...
    if args.id == "C262":
        return c262_datetime_subfamily_miner.run(forwarded)
    if args.id == "C263":
    ...
    if args.id == "C269":
        return c269_adaptive_length_gate_audit.run(forwarded)
    if args.id == "C270":
        return c270_adaptive_length_paired_aggregate.run(forwarded)
    if args.id == "C271":
        return c271_full_reference_sft_gate_audit.run(forwarded)
    if args.id == "C272":
    ...
    if args.id == "C296":
        return c296_strict_arithmetic_final_answer_route.run(forwarded)
    if args.id == "C299":
    ...
    if args.id == "C315":
        return c315_failure_gated_512_repetition_penalty.run(forwarded)
    if args.id == "C316":
        return c316_short_target_anchor_mixed_smoke.run(forwarded)
    if args.id == "C317":
        return c317_qwen3_30b_a3b_fp8_greedy.run(forwarded)
    if args.id == "C318":
        return c318_qwen3_30b_a3b_fp8_native_prompt.run(forwarded)
    if args.id == "C319":
    ...
    if args.id == "C333":
        return c333_failure_gated_mild_repetition_penalty.run(forwarded)
    if args.id == "C335":
    ...
    if args.id == "C343":
        return c343_compressed_reference_sft_ultramicro_cap_repair.run(forwarded)
