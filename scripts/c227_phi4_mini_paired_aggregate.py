...
EXPERIMENT_ID = "C227"
EXPERIMENT_SLUG = "C227_phi4_mini_paired_aggregate"
DEFAULT_OUT_DIR = Path("artifacts") / "tmp" / "C227_artifacts"
BASELINE_MODEL_ID = "Qwen/Qwen3-8B-AWQ"
VARIANT_MODEL_ID = "microsoft/Phi-4-mini-instruct"
MODEL_PACKAGE_METADATA = {
    "baseline_selected_files_gb": 6.10,
    "variant_selected_files_gb": 7.67,
    "metadata_source": "Hugging Face API with blobs=true, checked 2026-05-27",
}


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="C227 paired Qwen3-8B-AWQ vs Phi-4-mini aggregate.")
    ...
    summary.update(
        {
            ...
            "reason": "Paired Qwen3-8B-AWQ vs Phi-4-mini aggregate completed.",
            ...
            "variant_phi4_mini": variant,
            "delta_phi4_minus_8b": paired.overall_delta(variant, baseline),
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# C227 Phi-4-mini Paired Aggregate",
        "",
        "## Result",
        ...
        "",
        "## Delta Phi-4-mini Minus 8B",
        f"`{summary.get('delta_phi4_minus_8b')}`",
        "",
        "## Delta By Category",
        ...
        "",
        "## Variant Phi-4-mini",
        f"`{summary.get('variant_phi4_mini')}`",
        "",
        "## Hygiene",
