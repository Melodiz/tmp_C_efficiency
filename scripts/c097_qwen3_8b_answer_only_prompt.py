...
EXPERIMENT_ID = "C097"
EXPERIMENT_SLUG = "C097_qwen3_8b_answer_only_prompt"
DEFAULT_OUT_DIR = Path("artifacts") / "tmp" / "C097_artifacts"
ANSWER_ONLY_PREFIX = "На языке задания напиши только ответ. Без решения, пояснений, перевода и повторения условия."


def recommendation(metrics: dict[str, Any], dry_run: bool) -> tuple[str, str]:
    if dry_run:
        return "INVESTIGATE", "Dry run only; no answer-only prompt evidence was produced."
    validity = metrics.get("validity") or {}
    rates = metrics.get("rates") or {}
    projected = rates.get("projected_total_4000_min")
    if metrics.get("status") != "completed":
        return "INVESTIGATE", "The C097 runner did not complete."
    ...
    if int(validity.get("max_token_hit_rows") or 0) > 5:
        return "KILL", "Answer-only prompt did not reduce truncation versus the C093/C090 branch."
    if int(validity.get("repetition_loop_suspected_rows") or 0) > 3:
        return "KILL", "Answer-only prompt increased repetition risk."
    return "MUTATE", "Answer-only prompt passed validity gates; row-level review is needed before any merge."
...
def write_report(report_path: Path, metrics: dict[str, Any], args: Any, dry_run: bool) -> None:
    ...
    lines = [
        "# C097 Qwen3-8B Answer-Only Prompt Report",
        "",
        "## Objective",
        "- ID: C097",
        "- Mechanism: prompt-only test with a stricter answer-only prefix while keeping Qwen thinking disabled.",
        "- Leaderboard submission: NO.",
        "",
        "## Commands/config",
        f"- sample source: `{args.sample_source}`",
        f"- sample size: `{args.sample_size}`",
        f"- prefix: `{ANSWER_ONLY_PREFIX}`",
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    c096.THINKING_FINAL_ONLY_PREFIX = ANSWER_ONLY_PREFIX
    c096.OMIT_ENABLE_THINKING_FALSE = False
    c096.MECHANISM_ID = "answer_only_prompt_thinking_disabled"
