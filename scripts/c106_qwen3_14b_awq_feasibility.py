...
def configure_experiment() -> None:
    c078.EXPERIMENT_ID = "C106"
    c078.EXPERIMENT_SLUG = "C106_qwen3_14b_awq_feasibility"
    c078.DEFAULT_OUT_DIR = Path("artifacts") / "tmp" / "C106_artifacts"
    c078.MODEL_ID = MODEL_ID
    c078.QUANTIZATION = "awq_marlin"
    c078.SOURCE_EXPERIMENT_ID = "C104"
    c078.RUNNER_SCRIPT = "scripts/c106_qwen3_14b_awq_feasibility.py"
    c078.SAMPLING_CHANGED_FROM_C073 = True
    c078.MECHANISM_DESCRIPTION = (
        "replace only the Qwen3-8B-AWQ model with Qwen3-14B-AWQ while keeping "
        "the C093 short language-preserving prefix, greedy decoding, output cap, "
        "and no deterministic handlers."
    )
