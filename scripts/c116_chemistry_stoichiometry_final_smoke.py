...
EXPERIMENT_ID = "C116"
EXPERIMENT_SLUG = "C116_chemistry_stoichiometry_final_smoke"
DEFAULT_OUT_DIR = Path("artifacts") / "tmp" / "C116_artifacts"
...
def write_report(report_path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# C116 Chemistry Stoichiometry Final Smoke Report",
        "",
        "## Objective",
        "- ID: C116",
        "- Mechanism: final-entrypoint smoke for C113 plus strict chemistry stoichiometry exact solver.",
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    c088.SMOKE_ROWS = [
        ...
        {
            "rid": 2738,
            "question": "Сумма коэффициентов в уравнении реакции FeO + Zn → Fe + ZnO равна 1) 6 2) 9 3) 4 4) 7",
            "expected_contains": "4",
            "expected_exact": "4\n\nИтоговый ответ: 4",
        },
        {
            "rid": 6702,
            "question": "Рассчитайте объём водорода (при нормальных условиях) и массу азота, которые нужны для синтеза 95 л аммиака, если выход продукта реакции составляет 85 %.",
            "expected_contains": "167,6 л водорода и 69,9 г азота",
            "expected_exact": "167,6 л водорода и 69,9 г азота\n\nИтоговый ответ: 167,6 л водорода и 69,9 г азота",
        },
        {
            "rid": 8952,
            "question": "В реакции 3X + 2Y = Z начальные концентрации равны 0.8 и 0.6. Чему равна концентрация Y, если X станет 0.5?",
            "expected_contains": "0,4",
            "expected_exact": "0,4\n\nИтоговый ответ: 0,4",
        },
    ]
