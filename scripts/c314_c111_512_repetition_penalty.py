...
EXPERIMENT_ID = "C314"
EXPERIMENT_SLUG = "C314_c111_512_repetition_penalty"
DEFAULT_OUT_DIR = Path("artifacts") / "tmp" / "C314_artifacts"
MODEL_ID = "Qwen/Qwen3-8B-AWQ"
VARIANT_MAX_TOKENS = 512
REPETITION_PENALTY = 1.08


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="C314 compare C111 320 greedy vs 512 with repetition penalty.")
...
def run_validation(args: argparse.Namespace) -> dict[str, Any]:
    ...
    c111.MAX_NEW_TOKENS = VARIANT_MAX_TOKENS
    try:
        variant = c202.summarize_rows(c111, tokenizer, rows, variant_outputs)
    finally:
        c111.MAX_NEW_TOKENS = original_max_tokens
    ...
    summary.update(
        {
            ...
            "reason": "512-token repetition-penalty decode cleared local gate." if gate["pass"] else "512-token repetition-penalty decode did not clear local gate.",
            "raw_task_data_read_remote_only": True,
            "baseline_sampling": {"max_tokens": original_max_tokens, "temperature": 0.0, "top_p": 1.0, "top_k": -1},
            "sample_meta": {
                "rows": len(rows),
            ...
            "baseline_320_greedy": baseline,
            "variant_512_repetition_penalty": variant,
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# C314 C111 512 Repetition-Penalty Decode",
        "",
        "## Result",
        ...
        f"`{summary.get('delta_variant_minus_baseline')}`",
        "",
        "## Baseline 320 Greedy",
        f"`{summary.get('baseline_320_greedy')}`",
        "",
        "## Variant 512 Repetition Penalty",
        f"`{summary.get('variant_512_repetition_penalty')}`",
        "",
        "## Hygiene",
