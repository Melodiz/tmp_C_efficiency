...
EXPERIMENT_ID = "C326"
EXPERIMENT_SLUG = "C326_compressed_reference_sft_global_env_smoke"
DEFAULT_OUT_DIR = Path("artifacts") / "tmp" / "C326_artifacts"
MODEL_ID = c177.MODEL_ID


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="C326 compressed-reference SFT smoke using global Colab env.")
...
def run_smoke(args: argparse.Namespace, paths: dict[str, Path]) -> dict[str, Any]:
    ...
    summary: dict[str, Any] = {
        ...
        "setup": "global_env_no_target_install",
    }
    if args.dry_run:
    ...
    decision = "MUTATE" if ok and ref_delta > 0 and out_delta > 0 and cap_delta <= 0 and invalid_delta <= 0 else "KILL"
    summary.update(
        {
            "status": "completed" if ok else "failed",
            "decision_recommendation": decision if ok else "INVESTIGATE",
            "reason": "Global-env compressed-reference SFT smoke completed." if ok else "Global-env compressed-reference SFT smoke failed.",
            "probe_returncode": probe_code,
            "probe": probe_json,
            "data_meta": (probe_json or {}).get("data_meta"),
            "train": (probe_json or {}).get("train"),
            "validation": val,
            "containment_deltas": {"ref_in_output": ref_delta, "output_in_ref": out_delta, "cap": cap_delta, "invalid": invalid_delta},
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    text = text.replace("# C178 SFT Aggregate Metric/Cap Diagnostic", "# C326 Compressed-Reference SFT Global-Env Smoke", 1)
