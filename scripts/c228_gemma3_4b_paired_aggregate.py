...
EXPERIMENT_ID = "C228"
EXPERIMENT_SLUG = "C228_gemma3_4b_paired_aggregate"
DEFAULT_OUT_DIR = Path("artifacts") / "tmp" / "C228_artifacts"
BASELINE_MODEL_ID = "Qwen/Qwen3-8B-AWQ"
VARIANT_MODEL_ID = "google/gemma-3-4b-it"
MODEL_PACKAGE_METADATA = {
    "baseline_selected_files_gb": 6.10,
    "variant_selected_files_gb": 8.60,
    "variant_license": "gemma",
    "metadata_source": "Hugging Face API with blobs=true, checked 2026-05-27",
}


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="C228 paired Qwen3-8B-AWQ vs Gemma 3 4B aggregate.")
...
def run_validation(args: argparse.Namespace) -> dict[str, Any]:
    ...
    variant, variant_runtime = paired.run_model(VARIANT_MODEL_ID, c111, rows, args.seed, None)
    sampler.stop()
    ...
    summary.update(
        {
            ...
            "reason": "Paired Qwen3-8B-AWQ vs Gemma 3 4B aggregate completed.",
            ...
            "variant_gemma3_4b": variant,
            "delta_gemma3_minus_8b": paired.overall_delta(variant, baseline),
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# C228 Gemma 3 4B Paired Aggregate",
        "",
        "## Result",
        ...
        "",
        "## Delta Gemma 3 4B Minus 8B",
        f"`{summary.get('delta_gemma3_minus_8b')}`",
        "",
        "## Delta By Category",
        ...
        "",
        "## Variant Gemma 3 4B",
        f"`{summary.get('variant_gemma3_4b')}`",
        "",
        "## Hygiene",
