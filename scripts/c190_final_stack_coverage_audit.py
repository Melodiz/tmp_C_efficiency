...
EXPERIMENT_ID = "C190"
EXPERIMENT_SLUG = "C190_final_stack_coverage_audit"
DEFAULT_OUT_DIR = Path("artifacts") / "tmp" / "C190_artifacts"
DATA_PATH = Path("data/dataset_ml_challenge.parquet")


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="C190 final-stack deterministic coverage audit.")
    ...
    summary.update(
        {
            ...
            "reason": "Final-stack coverage audit completed." if ok else "Final-stack coverage audit failed.",
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# C190 Full-Distribution Final-Stack Coverage Audit",
        "",
        "## Objective",
    ...
    lines.extend(
        [
            ...
            "",
            "## Next",
            "Use full-distribution coverage to decide whether another broad solver audit is justified.",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
