...
import c097_qwen3_8b_answer_only_prompt as c097


EXPERIMENT_ID = "C098"
EXPERIMENT_SLUG = "C098_c097_with_c093_handlers_hard_audit"
DEFAULT_OUT_DIR = Path("artifacts") / "tmp" / "C098_artifacts"
...
def run_source_c097(out_dir: Path, args: argparse.Namespace) -> Path:
    source_out = out_dir.parent / f"{out_dir.name}_source_c097_{base.utc_stamp()}"
    ...
    code = c097.run(forwarded)
    if code != 0:
        raise RuntimeError(f"C097 source runner failed with exit {code}")
    return source_out


def apply_handlers(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    expr_rows, expr_stats = c083.apply_guard(rows)
    post_rows, post_stats = c086.apply_postprocess(expr_rows)
    c089.cleanup_english_answer = c090.strict_cleanup_english_answer
    final_rows, english_stats = c089.apply_cleanup(post_rows)
    return final_rows, {
        "expression_guard": expr_stats,
        "comma_repetition_dedup": post_stats,
...
def build_summary(source_summary: dict[str, Any], rows: list[dict[str, Any]], handler_stats: dict[str, Any], paths: dict[str, Path]) -> dict[str, Any]:
    ...
    config.update(
        {
            "c098_mechanism": "c097_prompt_with_existing_c093_handlers",
            "source_experiment_id": "C097",
    ...
    summary["tokens"] = {
        **(summary.get("tokens") or {}),
        "avg_output_tokens_after_handlers": sum(output_token_counts) / max(1, len(output_token_counts)),
        "max_output_tokens_after_handlers": max(output_token_counts) if output_token_counts else None,
        "total_output_tokens_after_handlers": sum(output_token_counts),
    }
    summary["validity"] = {
        "jsonl_rows": len(rows),
...
def recommendation(metrics: dict[str, Any], dry_run: bool) -> tuple[str, str]:
    ...
    if metrics.get("status") != "completed":
        return "INVESTIGATE", "The C098 runner did not complete."
    ...
    if int(validity.get("max_token_hit_rows") or 0) > 3:
        return "KILL", "Hard-audit truncation did not improve enough versus C092/C093."
    if int(validity.get("repetition_loop_suspected_rows") or 0) > 2:
        return "KILL", "Repetition risk remains too high."
    return "MUTATE", "C097 prompt plus existing handlers passed hard-audit validity gates; row-level review and final smoke are needed."
...
def write_report(report_path: Path, metrics: dict[str, Any], args: argparse.Namespace, dry_run: bool) -> None:
    ...
    lines = [
        "# C098 C097 With C093 Handlers Hard-Audit Report",
        "",
        "## Objective",
        "- ID: C098",
        "- Mechanism: validate C097 answer-only prompt with the existing C093 handler stack.",
...
def create_dry_run(paths: dict[str, Path], args: argparse.Namespace) -> dict[str, Any]:
    run_id = f"{base.utc_stamp()}_c098_dry_run"
    run_paths = {
        **paths,
...
def create_gpu_artifacts(paths: dict[str, Path], args: argparse.Namespace) -> dict[str, Any]:
    ...
    source_out = run_source_c097(paths["out_dir"], args)
    ...
    processed_rows, handler_stats = apply_handlers(source_outputs)
    run_id = str(source_summary.get("run_id") or f"{base.utc_stamp()}_c098")
    run_paths = {
        **paths,
        "summary": paths["results_dir"] / f"{run_id}.summary.json",
        "metrics": paths["results_dir"] / f"{run_id}.metrics.json",
    ...
    summary.setdefault("runtime", {})["c098_wrapper_s"] = time.perf_counter() - started
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run C098 C097 prompt with existing C093 handlers on hard-audit.")
