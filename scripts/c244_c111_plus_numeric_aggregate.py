...
def c111_plus_numeric_stack(current: Any, c111: Any, question: str, answer: str) -> tuple[str, str]:
    ...
    numeric = current.exact_numeric_answer(question)
    if numeric is not None:
        return numeric, "exact_numeric"
    return c111_answer, "fallback_model"


def run_validation(args: Any) -> dict[str, Any]:
    base.c111_plus_formulaic_stack = c111_plus_numeric_stack
    summary = _BASE_RUN_VALIDATION(args)
    summary["experiment_id"] = base.EXPERIMENT_ID
    summary["experiment_slug"] = base.EXPERIMENT_SLUG
    summary["mechanism"] = "C111 plus only exact_numeric_answer on C111 fallback rows"
    if summary.get("reason") == "C111 plus isolated formulaic solver aggregate completed.":
        summary["reason"] = "C111 plus isolated numeric solver aggregate completed."
    if "formulaic_quality" in summary:
        summary["numeric_quality"] = summary["formulaic_quality"]
    if "delta_formulaic_minus_c111" in summary:
        summary["delta_numeric_minus_c111"] = summary["delta_formulaic_minus_c111"]
    return summary


def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# C244 C111 Plus Isolated Numeric Solver Aggregate",
        "",
        "## Result",
        ...
        f"- C111 plus numeric: `{summary.get('numeric_quality')}`",
        f"- delta numeric minus C111: `{summary.get('delta_numeric_minus_c111')}`",
        "",
        "## Changed Rows",
...
def run(argv: Sequence[str] | None = None) -> int:
    base.EXPERIMENT_ID = "C244"
    base.EXPERIMENT_SLUG = "C244_c111_plus_numeric_aggregate"
    base.DEFAULT_OUT_DIR = Path("artifacts") / "tmp" / "C244_artifacts"
    base.c111_plus_formulaic_stack = c111_plus_numeric_stack
