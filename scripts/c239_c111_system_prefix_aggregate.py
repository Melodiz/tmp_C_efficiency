...
EXPERIMENT_ID = "C239"
EXPERIMENT_SLUG = "C239_c111_system_prefix_aggregate"
DEFAULT_OUT_DIR = Path("artifacts") / "tmp" / "C239_artifacts"
MODEL_ID = "Qwen/Qwen3-8B-AWQ"


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="C239 compare C111 user-prefix vs system-prefix structure.")
...
def apply_system_prefix_template(tokenizer: Any, question: str, system_prefix: str) -> str:
    messages = [
...
def run_validation(args: argparse.Namespace) -> dict[str, Any]:
    ...
    system_prompts = [
        apply_system_prefix_template(tokenizer, str(row["question"]), c111.USER_PREFIX) for row in rows
    ]
    ...
    summary.update(
        {
            ...
            "reason": "C111 user-prefix vs system-prefix aggregate completed.",
            ...
            "variant_system_prefix": system,
            "delta_system_minus_user": delta(system, control),
            "model_loaded": True,
        }
    )
    return summary
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# C239 C111 System-Prefix Aggregate",
        "",
        "## Result",
        ...
        "",
        "## Delta System Minus User",
        f"`{summary.get('delta_system_minus_user')}`",
        "",
        "## User-Prefix Control",
        f"`{summary.get('control_user_prefix')}`",
        "",
        "## System-Prefix Variant",
        f"`{summary.get('variant_system_prefix')}`",
        "",
        "## Hygiene",
