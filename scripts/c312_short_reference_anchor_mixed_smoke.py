...
EXPERIMENT_ID = "C312"
EXPERIMENT_SLUG = "C312_short_reference_anchor_mixed_smoke"
...
def task_probe_source(
    ...
    source = source.replace('data["answer_len"] <= 3600', 'data["answer_len"] <= 800')
    return source
...
def write_report(path: Path, summary: dict) -> None:
    ...
    text = text.replace("# C301 Short-Prefix Anchor-Mixed Micro", "# C312 Short-Reference Anchor-Mixed Smoke", 1)
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    c177.DEFAULT_OUT_DIR = Path("artifacts") / "tmp" / "C312_artifacts"
    c177.DEFAULT_TARGET_DIR = Path("/content/c312_train_site")
    c177.REMOTE_ADAPTER_DIR = Path("/content/c312_adapter_scratch")
