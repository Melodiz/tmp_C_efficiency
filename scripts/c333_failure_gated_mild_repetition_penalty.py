...
EXPERIMENT_ID = "C333"
EXPERIMENT_SLUG = "C333_failure_gated_mild_repetition_penalty"
DEFAULT_OUT_DIR = Path("artifacts") / "tmp" / "C333_artifacts"
MODEL_ID = "Qwen/Qwen3-8B-AWQ"
REPETITION_PENALTY = 1.03


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="C333 route visible C111 failures to 320-token mild repetition-penalty fallback.")
...
def run_validation(args: argparse.Namespace) -> dict[str, Any]:
    summary: dict[str, Any] = {
        ...
        "model_id": MODEL_ID,
        "route": "Use 320-token mild repetition-penalty fallback only when C111 output hits max tokens or repetition-loop flags; accept only if fallback is visibly valid.",
        "fallback_sampling": {
            "max_tokens": None,
            "temperature": 0.0,
            "top_p": 1.0,
    ...
    if routed_prompts:
        ...
        fallback_eval = [
            c246.row_eval(c111, tokenizer, row, out, c111.MAX_NEW_TOKENS)
    ...
    summary.update(
        {
            ...
            "reason": "Failure-gated mild repetition penalty cleared local gate." if gate["pass"] else "Failure-gated mild repetition penalty failed local gate.",
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# C333 Failure-Gated Mild Repetition Penalty",
        "",
        "## Result",
