...
EXPERIMENT_ID = "C235"
EXPERIMENT_SLUG = "C235_c111_max_tokens_512"
DEFAULT_OUT_DIR = Path("artifacts") / "tmp" / "C235_artifacts"
MODEL_ID = "Qwen/Qwen3-8B-AWQ"
VARIANT_MAX_TOKENS = 512


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="C235 compare C111 max_tokens 320 vs 512.")
...
def run_validation(args: argparse.Namespace) -> dict[str, Any]:
    summary: dict[str, Any] = {
        ...
        "variant_max_tokens": VARIANT_MAX_TOKENS,
    }
    if args.dry_run:
    ...
    variant_sampling = SamplingParams(temperature=0.0, max_tokens=VARIANT_MAX_TOKENS, top_p=1.0, top_k=-1)
    ...
    summary.update(
        {
            ...
            "reason": "C111 max_tokens 320 vs 512 paired aggregate completed.",
            ...
            "variant_512": variant,
            "delta_512_minus_320": delta(variant, baseline),
            "model_loaded": True,
        }
    )
    return summary
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# C235 C111 Max-Tokens 512 Paired Aggregate",
        "",
        "## Result",
        ...
        f"- variant max tokens: `{summary.get('variant_max_tokens')}`",
        ...
        "",
        "## Delta 512 Minus 320",
        f"`{summary.get('delta_512_minus_320')}`",
        "",
        "## Baseline 320",
        f"`{summary.get('baseline_320')}`",
        "",
        "## Variant 512",
        f"`{summary.get('variant_512')}`",
        "",
        "## Hygiene",
