...
def probe_source() -> str:
    return textwrap.dedent(
        ...
        SUBFAMILIES = {{
            "chem_balancing_equation": [r"(?:уравнен|коэффициент|расстав|баланс|reaction|equation)", r"(?:->|→|=)"],
            "chem_formula_substance": [r"(?:формул|веществ|соединен|оксид|кислот|соль|formula|compound|oxide|acid)"],
            "chem_mole_molar_mass": [r"(?:моль|моляр|масса|объем|количеств[оа]\\s+веществ|mole|molar|mass|volume)"],
            "chem_element_atom_ion": [r"(?:элемент|атом|ион|протон|электрон|нейтрон|валентн|element|atom|ion)"],
            "geom_circle": [r"(?:окружн|круг|радиус|диаметр|дуг[аи]|circle|radius|diameter|arc)"],
            "geom_triangle": [r"(?:треугольн|катет|гипотенуз|медиан|биссектрис|triangle|hypotenuse)"],
            "geom_area_perimeter": [r"(?:площад|периметр|area|perimeter)"],
            "geom_coordinate": [r"(?:координат|центр|точк[аи]|прям[ао]й|наклон|coordinate|slope|center|point)"],
            "geom_angle": [r"(?:угол|градус|angle|degree)"],
        }}
