...
EXPERIMENT_ID = "C307"
EXPERIMENT_SLUG = "C307_qwen3_4b_2507_recommended_sampling"
DEFAULT_OUT_DIR = Path("artifacts") / "tmp" / "C307_artifacts"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="C307 Qwen3-4B-2507-FP8 recommended sampling aggregate.")
...
def run_variant_model(model_id: str, c111: Any, rows: list[dict[str, Any]], seed: int) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    sampling = SamplingParams(
        temperature=RECOMMENDED_SAMPLING["temperature"],
        top_p=RECOMMENDED_SAMPLING["top_p"],
        top_k=RECOMMENDED_SAMPLING["top_k"],
        min_p=RECOMMENDED_SAMPLING["min_p"],
...
def run_validation(args: argparse.Namespace) -> dict[str, Any]:
    summary: dict[str, Any] = {
        ...
        "variant_sampling": RECOMMENDED_SAMPLING,
    }
    if args.dry_run:
    ...
    variant, variant_runtime = run_variant_model(VARIANT_MODEL_ID, c111, rows, args.seed)
    sampler.stop()
    ...
    summary.update(
        {
            ...
            "reason": "Recommended sampling 4B-2507 cleared local gates." if gate["pass"] else "Recommended sampling 4B-2507 did not clear local gates.",
            ...
            "baseline_8b_awq_greedy": baseline,
            "variant_4b_2507_fp8_recommended_sampling": variant,
            "delta_4b_recommended_minus_8b": delta,
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        ...
        f"`{summary.get('gate')}`",
        "",
        "## Delta 4B Recommended Minus 8B",
        f"`{summary.get('delta_4b_recommended_minus_8b')}`",
        "",
        "## Delta By Category",
        ...
        "",
        "## Baseline 8B",
        f"`{summary.get('baseline_8b_awq_greedy')}`",
        "",
        "## Variant 4B Recommended",
        f"`{summary.get('variant_4b_2507_fp8_recommended_sampling')}`",
        "",
        "## Hygiene",
