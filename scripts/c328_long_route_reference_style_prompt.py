...
EXPERIMENT_ID = "C328"
EXPERIMENT_SLUG = "C328_long_route_reference_style_prompt"
DEFAULT_OUT_DIR = Path("artifacts") / "tmp" / "C328_artifacts"
MODEL_ID = "Qwen/Qwen3-8B-AWQ"
REFERENCE_STYLE_PREFIX = (
    "Ответь на языке задания. Для вопросов, где нужен развернутый ответ, дай полный "
    "учебный ответ: определение, краткое объяснение и структурированный список ключевых пунктов. "
    "Не добавляй лишних рассуждений и не повторяй условие."
)


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="C328 long-route reference-style prompt aggregate.")
...
def run_validation(args: argparse.Namespace) -> dict[str, Any]:
    summary: dict[str, Any] = {
        ...
        "model_id": MODEL_ID,
        "mechanism": "C269 long-answer route uses reference-style prompt; non-routed rows preserve C111 prompt; max_tokens remains 320",
    }
    if args.dry_run:
    ...
    variant_prompts = [
        probe.apply_user_only_template(
            ...
            str(row["question"]),
            True,
            REFERENCE_STYLE_PREFIX if idx in routed_indices else c111.USER_PREFIX,
        )
    ...
    baseline = c270.summarize_rows_with_caps(c111, tokenizer, rows, baseline_outputs, [c111.MAX_NEW_TOKENS] * len(rows))
    variant = c270.summarize_rows_with_caps(c111, tokenizer, rows, variant_outputs, [c111.MAX_NEW_TOKENS] * len(rows))
    ...
    summary.update(
        {
            ...
            "reason": "Long-route reference-style prompt cleared local gate." if gate["pass"] else "Long-route reference-style prompt failed local gate.",
            ...
            "variant_reference_style": variant,
            "delta_reference_style_minus_c111": overall_delta,
            "routed_baseline_c111": routed_baseline,
            "routed_variant_reference_style": routed_variant,
            "routed_delta_reference_style_minus_c111": routed_delta,
            "success_gate": gate,
        }
    )
    return summary
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# C328 Long-Route Reference-Style Prompt",
        "",
        "## Result",
        ...
        "",
        "## Delta Reference Style Minus C111",
        f"`{summary.get('delta_reference_style_minus_c111')}`",
        "",
        "## Routed Delta Reference Style Minus C111",
        f"`{summary.get('routed_delta_reference_style_minus_c111')}`",
        "",
        "## Baseline C111",
        f"`{summary.get('baseline_c111')}`",
        "",
        "## Variant Reference Style",
        f"`{summary.get('variant_reference_style')}`",
        "",
        "## Hygiene",
